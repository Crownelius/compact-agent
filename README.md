...
Long-horizon profiles (`roadmapbench`, `saasbench`, `swe-bench-mobile`, and `webdevbench`) add milestone-oriented prompts and trace fields for roadmap/SaaS/mobile/WebDevBench coverage risk. The WebDevBench profile follows the SWE-WebDevBench paper and benchmark resources by preserving canary business requirements, distinguishing app creation from modification, and requiring frontend-backend plus production/security evidence when feasible. The trace warns when these tasks lack a post-context milestone or canary checklist, leave roadmap items incomplete after visible validation, or claim completion without broad integration, platform, migration, e2e, frontend-backend, production-readiness, security, or repeated validation evidence.
