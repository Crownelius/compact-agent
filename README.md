...
- Automatic repo-map context for larger codebases: Cawdex injects a bounded symbol/file outline ranked by local dependency references and request terms, then asks the model to read full files before editing. `/context dossier <task>` gives the same workflow a no-model local candidate-file/verifier starting point, and `/lifespan` diagnoses long-session compression/interference/revision/maintenance risk before old context bleeds into new work.
