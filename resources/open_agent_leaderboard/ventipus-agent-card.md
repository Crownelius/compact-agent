...
Benchmark traces write compact `experienceCard` summaries with replay checkpoints, failure signatures, task-contract state and signals, environment-reconstruction setup/failure evidence, dependency-upgrade setup-validation evidence, decision-observability edit predictions, validation-reliability evidence, source-research coverage, verification commands, changed files, and warnings.
