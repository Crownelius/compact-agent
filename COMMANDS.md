...
AHE change evaluation expects each non-trivial benchmark edit to carry both `Prediction:` and `At-risk regression:` manifest lines. Failed-verifier repair edits should also carry a `Root cause:`, `Diagnosis:`, or failure-tied `Hypothesis:` line plus a `Targeted fix:` or `Fix plan:` line before patching. Missing regression forecasts, root-cause hypotheses, and targeted-fix manifests are reported in trace summaries, `change-evaluation.json` or `experienceCard.rootCauseHypothesis` as applicable, process defects, and completion reminders.
