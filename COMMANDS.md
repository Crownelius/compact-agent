...
Long-horizon profiles (`roadmapbench`, `saasbench`, `swe-bench-mobile`, and `webdevbench`) add roadmap/SaaS/mobile/WebDevBench coverage checks to benchmark traces and KBench/HAL adapter routing. Trace summaries flag missing milestone or canary checklists, incomplete roadmap items after validation, and absent broad integration/platform/frontend-backend/security validation for RoadmapBench, SaaSBench, SWE-Bench Mobile, and SWE-WebDevBench-style work.
