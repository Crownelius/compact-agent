...
| `benchmark_repo_catalog` | Offline Terminal-Bench 2.0 public repository seed catalog for source mining; returns verified direct mappings, related/partial mappings, and known no-public-source gaps, and reminds the agent to verify positive hits with `github_repo_digest` before porting patterns. | Yes |
