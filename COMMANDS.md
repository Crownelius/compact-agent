...
| `VENTIPUS_BENCHMARK_TRACE_DIR` | `~/.ventipus/benchmark-runs` | Directory for benchmark `summary.json`, `trace.jsonl`, `open-agent-leaderboard-draft.json`, and git `worktree.patch` / `git-status.txt` artifacts. `summary.json` includes a compact `experienceCard` for future replay/context reuse with bounded task-contract signals used for prior-run relevance ranking; patch output includes unstaged, staged, and untracked file diffs where git can render them. |
