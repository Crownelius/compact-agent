...
Open Agent general-task profiles (`appworld`, `browsecomp`, and `tau2`) tune `/benchmark` prompts for stateful app/API actions, source-grounded web research, and policy-bound customer workflows. The Exgentic adapter auto-selects these profiles from task/context/action schemas and prints available action names separately to reduce invalid action selection.
