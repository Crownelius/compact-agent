...
Open Agent general-task profiles (`appworld`, `browsecomp`, and `tau2`) tune `/benchmark` prompts for stateful app/API actions, source-grounded web research, and policy-bound customer workflows. The Exgentic adapter auto-selects these profiles from task/context/action schemas, prints available action names separately, adds a deterministic current-state action shortlist before the full schemas, and repairs case/camelCase/schema-key near misses plus exact latest-observation/context required-argument omissions before dispatch to reduce invalid or premature action selection.
