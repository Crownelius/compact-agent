...
AHE change evaluation expects each non-trivial benchmark edit to carry both `Prediction:` and `At-risk regression:` manifest lines. Failed-verifier repair edits should also carry a `Root cause:`, `Diagnosis:`, or failure-tied `Hypothesis:` line before patching. Missing regression forecasts and root-cause hypotheses are reported in trace summaries, `change-evaluation.json` or `experienceCard.rootCauseHypothesis` as applicable, process defects, and completion reminders.
