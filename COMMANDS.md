...
The `pi-bench` profile is for proactive personal assistant benchmark tasks. It keeps a context contract for user profile, current request, message/file/app/workspace context, domain tools, hidden-intent hypotheses, privacy risk, and clarification state before acting. Hidden intents, gold answers, private rubrics, expected traces, and result files are treated as oracle material.
