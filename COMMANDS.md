...
| `/benchmark [profile] <task>` | Benchmark-grade issue/terminal/general-agent workflow. Profiles: `swe-bench`, `terminal-bench`, `swe-context`, `swe-chain`, `swe-ci`, `ci-repair`, `wildclaw`, `arc-agi`, `specbench`, `reward-hacking`, `roadmapbench`, `saasbench`, `swe-bench-mobile`, `appworld`, `browsecomp`, `tau2`, `generic`. Aliases: `/bench`, `/leaderboard`. |
...
Long-horizon profiles (`roadmapbench`, `saasbench`, `swe-bench-mobile`, `webdevbench`, and `swe-ci`) add roadmap/SaaS/mobile/WebDevBench/SWE-CI coverage checks to benchmark traces and KBench/HAL adapter routing. Trace summaries flag missing milestone, canary, or evolution/test-gap checklists, incomplete roadmap items after validation, and absent broad integration/platform/frontend-backend/security/CI-loop validation for RoadmapBench, SaaSBench, SWE-Bench Mobile, SWE-WebDevBench, and SWE-CI-style work.
