...
| `/benchmark [profile] <task>` | Benchmark-grade issue/terminal/general-agent workflow. Profiles: `swe-bench`, `terminal-bench`, `swe-context`, `swe-chain`, `swe-cycle`, `swe-ci`, `swe-prbench`, `tml-bench`, `pi-bench`, `ci-repair`, `wildclaw`, `arc-agi`, `specbench`, `reward-hacking`, `roadmapbench`, `saasbench`, `swe-bench-mobile`, `appworld`, `browsecomp`, `tau2`, `generic`. Aliases: `/bench`, `/leaderboard`. |
...
| `/benchmark-repos [query] [flags]` | Offline public Terminal-Bench repo catalog from the packaged research seed. Flags: `--all`, `--official`, `--related`, `--limit`, `--repos-only`. |
...
| `/harness [component]` | Show an AHE-style harness component map with files, tests, docs, and edit contracts. Components: `system_prompts`, `tooling`, `middleware`, `skills`, `subagents`, `memory`, `providers`, `benchmark_adapters`, `cli_ui`, `verification`. |
