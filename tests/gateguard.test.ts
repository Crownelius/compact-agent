...
// inherits HOME or CAWDEX_* from the test runner's environment.
...
describe('gateguard hook', () => {
  ...
  describe('yolo bypass (v1.27.1)', () => {
    it('returns ok() when CAWDEX_PERMISSION_MODE=yolo', () => {
      const r = runHook('gateguard', {
        CAWDEX_PERMISSION_MODE: 'yolo',
        CAWDEX_TOOL_INPUT: JSON.stringify({ file_path: existingFile }),
        CAWDEX_SESSION_ID: 'test-session',
      });
      expect(r.exitCode).toBe(0);
      expect(r.stderr).toBe('');
    });

    it('keeps CAWDEX_PERMISSION_MODE as the hook contract', () => {
      const r = runHook('gateguard', {
        CAWDEX_PERMISSION_MODE: 'yolo',
        CAWDEX_TOOL_INPUT: JSON.stringify({ file_path: existingFile }),
        CAWDEX_SESSION_ID: 'test-session',
    ...
    it('does NOT bypass when permission mode is anything else (strict mode)', () => {
      ...
      const r = runHook('gateguard', {
        CAWDEX_PERMISSION_MODE: 'ask',
        CAWDEX_GATEGUARD_MODE: 'block',
        CAWDEX_TOOL_INPUT: JSON.stringify({ file_path: existingFile }),
        CAWDEX_SESSION_ID: `test-${Date.now()}`,
      });
  ...
  describe('env-var disable knob (v1.27.1)', () => {
    for (const val of ['off', 'OFF', 'false', '0', 'no', 'disabled']) {
      it(`returns ok() when CAWDEX_GATEGUARD=${val}`, () => {
        const r = runHook('gateguard', {
          CAWDEX_GATEGUARD: val,
          CAWDEX_TOOL_INPUT: JSON.stringify({ file_path: existingFile }),
          CAWDEX_SESSION_ID: `test-${Date.now()}`,
        });
        expect(r.exitCode).toBe(0);
  ...
  describe('non-existent file bypass (v1.27.2)', () => {
    it('returns ok() for a file that does not exist (warn mode)', () => {
      const r = runHook('gateguard', {
        CAWDEX_TOOL_INPUT: JSON.stringify({ file_path: nonExistentFile }),
        CAWDEX_SESSION_ID: `test-${Date.now()}`,
      });
      expect(r.exitCode).toBe(0);
  ...
  describe('sessionId path-traversal sanitization (v1.28.1)', () => {
    // The state file is written under ~/.cawdex/state/gateguard/
    // <sessionId>.json. Without sanitization, a sessionId of
    // "../../../escape" would write outside that dir.
    const stateDir = join(homedir(), '.cawdex', 'state', 'gateguard');
    ...
    it('falls back to "unknown" on a path-traversal sessionId', () => {
      ...
      const r = runHook('gateguard', {
        CAWDEX_GATEGUARD_MODE: 'block',
        CAWDEX_TOOL_INPUT: JSON.stringify({ file_path: uniqueFile }),
        CAWDEX_SESSION_ID: '../../../escape',
      });
      ...
      const traversalTarget = join(homedir(), '.cawdex', 'state', 'escape.json');
  ...
  describe('malformed tool input (v1.28.1 fail-closed fix)', () => {
    it('blocks when CAWDEX_TOOL_INPUT is non-empty malformed JSON', () => {
      const r = runHook('gateguard', {
        CAWDEX_TOOL_INPUT: 'not valid json {{{',
        CAWDEX_SESSION_ID: 'test',
      });
      expect(r.exitCode).toBe(2);
