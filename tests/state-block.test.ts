 ...
 *   - Respects COMPACT_AGENT_STATE_BLOCK=0 to opt out
 */
import { describe, it, expect, afterEach } from 'vitest';
import { buildStateBlock } from '../src/query.js';
import type { Message } from '../src/types.js';

const ORIGINAL = process.env.COMPACT_AGENT_STATE_BLOCK;
afterEach(() => {
  if (ORIGINAL === undefined) delete process.env.COMPACT_AGENT_STATE_BLOCK;
  else process.env.COMPACT_AGENT_STATE_BLOCK = ORIGINAL;
});
...
describe('buildStateBlock', () => {
  ...
  it('returns null when COMPACT_AGENT_STATE_BLOCK=0', () => {
    process.env.COMPACT_AGENT_STATE_BLOCK = '0';
    const msgs: Message[] = [
      { role: 'user', content: 'do thing' },
