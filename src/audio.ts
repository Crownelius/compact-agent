...
export function micProbeMessage(r: MicProbeResult): string {
  switch (r) {
    ...
    case 'no-mic':    return 'No microphone detected. Plug one in or set CAWDEX_AUDIO_INPUT to a specific device.';
  ...
  return new Promise<Buffer | null>((resolve) => {
    ...
    child.on('close', (code) => {
      if (code !== 0 && chunks.length === 0) {
        if (process.env.CAWDEX_AUDIO_DEBUG) {
