...
function buildBenchmarkSubmissionBundleManifest(
  ...
  if (options.tracePath && typeof options.traceText === 'string') {
    artifacts.push({
      ...
      description: 'Raw redacted benchmark event trace written by Cawdex.',
...
export function writeBenchmarkTrace(input: BenchmarkTraceWriteInput): BenchmarkTraceWriteResult | null {
  ...
  summary.artifacts.push({
    ...
    contentType: 'application/json',
    description: 'AHE-style change manifest evaluation from Cawdex edit predictions, at-risk regression forecasts, verifier outcomes, and post-edit regression cycles.',
