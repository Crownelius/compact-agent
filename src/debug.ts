 ...
 *
 * Writes NDJSON event records to ~/.cawdex/debug/<sessionId>.jsonl
 ...
 *   2. $CAWDEX_DEBUG env var
 *   3. existing state.level (typically 'off')
 *
 * Sets the log file path under ~/.cawdex/debug/<sessionId>.jsonl
...
export function initDebug(sessionId: string, explicitLevel?: DebugLevel | string): void {
  ...
  const envLevel = process.env.CAWDEX_DEBUG;
