...
export function getVoiceConfig(config: CrowcoderConfig): VoiceConfig {
  return config.voice || {};
}

export function isVoiceEnabled(config: CrowcoderConfig): boolean {
  return Boolean(config.voice?.enabled);
}

export function getSttKey(config: CrowcoderConfig): string {
...
export function getTtsKey(config: CrowcoderConfig): string {
  // ElevenLabs key — no fallback; this is a separate provider.
  return config.voice?.tts?.apiKey || '';
}

export function getSttConfig(config: CrowcoderConfig): Required<Omit<VoiceSttConfig, 'apiKey'>> & { apiKey: string } {
...
export function getTtsConfig(config: CrowcoderConfig): Required<Omit<VoiceTtsConfig, 'apiKey'>> & { apiKey: string } {
...
export function getAccessibilityConfig(config: CrowcoderConfig): Required<VoiceAccessibilityConfig> {
 ...
 */
export function printVoiceStatus(cfg: CrowcoderConfig): void {
