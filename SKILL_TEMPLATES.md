...
5. **Check hooks** — Verify hooks seeded in `~/.cawdex/hooks.json`.
...
**Steps**:
1. **Read current config** — `read_file` on `~/.cawdex/config.json` or use `/config`.
...
**Steps**:
1. **List current hooks** — `list_hooks` or read `~/.cawdex/hooks.json`.
